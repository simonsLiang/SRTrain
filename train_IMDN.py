...
testset = Set5_val.DatasetFromFolderVal("Test_Datasets/Set5/",
                                       ...
                                       args.scale)
tset = Set5_val.DatasetFromFolderVal("Train_Datasets/Set5/",
                                       "Train_Datasets/Set5_LR/x{}/".format(args.scale),
                                       args.scale)
...
testing_data_loader = DataLoader(dataset=testset, num_workers=args.threads, batch_size=args.testBatchSize,
                                 shuffle=False)
t_data_loader = DataLoader(dataset=tset, num_workers=args.threads, batch_size=args.testBatchSize,
                                 shuffle=False)                                 
...
def valid_t():
    ...
    for batch in t_data_loader:
        ...
        avg_psnr += utils.compute_psnr(im_pre, im_label)
        avg_ssim += utils.compute_ssim(im_pre, im_label)
    print("===> Val_t. psnr: {:.4f}, ssim: {:.4f}".format(avg_psnr / len(testing_data_loader), avg_ssim / len(testing_data_loader)))

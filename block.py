...
def conv_layer(in_channels, out_channels, kernel_size, stride=1, dilation=1, groups=1, bias=True):
    if kernel_size==5:
      return ACBlock(in_channels, out_channels)
    else:
...
class ACBlock(nn.Module):
    def __init__(self, in_channels, out_channels):
        ...
        self.conv3x3 = nn.Conv2d(in_channels, out_channels, (3, 3), 1, (1, 1),bias=False)
    ...
    def merge(self):
      with torch.no_grad():
        weight1 = self.conv1x3.weight.data
        weight2 = self.conv3x1.weight.data
        weight3 = self.conv3x3.weight.data
        new_weights = weight3
        for i in range(len(weight1[0])):
          new_weights[0][i][1] = (new_weights[0][i][1]+weight1[0][i])
        for i in range(len(weight1[1])):
          new_weights[1][i][1] = new_weights[1][i][1]+weight1[1][i]
        for i in range(len(weight2[0])):
          new_weights[0][i][0][1] = (new_weights[0][i][0][1]+weight2[0][i][0])
          new_weights[0][i][1][1] = (new_weights[0][i][1][1]+weight2[0][i][1])
          new_weights[0][i][2][1] = (new_weights[0][i][2][1]+weight2[0][i][2])
        for i in range(len(weight2[1])):
          new_weights[1][i][0][1] = (new_weights[1][i][0][1]+weight2[1][i][0])
          new_weights[1][i][1][1] = (new_weights[1][i][1][1]+weight2[1][i][1])
          new_weights[1][i][2][1] = (new_weights[1][i][2][1]+weight2[1][i][2])
        for p in self.conv3x3.parameters():
          p.data = new_weights
